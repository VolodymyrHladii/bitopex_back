...
import { AwsHelper } from "../heplers/aws.helper";
...
export class AWSController {
  static fileUpload = catchAsync(async (req: any, res: Response, next: NextFunction) => {
    try {
      ...
      const uploadedFile = await AwsHelper.uploadFile(file.buffer, `bitopex/assets/task-uploads/${Date.now() + "_" + file.originalname}`, {
